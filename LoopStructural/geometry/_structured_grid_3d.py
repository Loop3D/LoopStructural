...
class StructuredGrid3DGeometry:
    ...
    ``nsteps`` here is a node count (matching the convention used by
    :class:`LoopStructural.geometry.StructuredGrid`/``BoundingBox``). Callers
    that accept a cell count (e.g. ``BaseStructuredSupport``) are responsible
    for translating cells -> nodes before constructing this class.
    """
    ...
    def origin(self, origin):
        ...
        if np.any(~(self._nsteps > 0)):
            logger.error(
                f"Cannot resize the grid. The proposed number of steps is {self._nsteps}, these must be all > 0"
            )
