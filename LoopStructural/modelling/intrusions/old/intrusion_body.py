...
class IntrusionBody:
    ...
    def set_data_for_g_simulation(self):
        ...
        ----------
        """
        intrusion_network_data_xyz = self.intrusion_network.intrusion_network_data.loc[
            :, ["X", "Y", "Z"]
        ].to_numpy()
        intrusion_network_data = self.intrusion_network.intrusion_network_data.loc[
            :, ["X", "Y", "Z"]
        ].copy()
        ...
        other_contact_data_xyz = self.intrusion_network.other_contact_data.loc[
            :, ["X", "Y", "Z"]
        ].to_numpy()
        other_contact_data = self.intrusion_network.other_contact_data.loc[
            :, ["X", "Y", "Z"]
        ].copy()
    ...
    def simulate_s_thresholds(self):
        ...
        data_conceptual_minS = self.lateral_extent_model(
            data_minS, minP=minP, maxP=maxP, minS=minS, maxS=maxS
        )
        data_residual_minS = (
            data_conceptual_minS[:, 1] - data_minS.loc[:, "coord2"]
        ).to_numpy()
        inputsimdata_minS = data_minS.loc[
        ...
        data_conceptual_maxS = self.lateral_extent_model(
            data_maxS, minP=minP, maxP=maxP, minS=minS, maxS=maxS
        )
        data_residual_maxS = (
            data_conceptual_maxS[:, 0] - data_maxS.loc[:, "coord2"]
        ).to_numpy()
        inputsimdata_maxS = data_maxS.loc[
            :, ["X", "Y", "Z", "coord0", "coord1", "coord2"]
        ].copy()
        ...
        if self.simulation_s_parameters.get("zmin") == None:
            self.simulation_s_parameters["zmin"] = min(
                inputsimdata_maxS["s_residual"].min(),
                inputsimdata_minS["s_residual"].min(),
            )
        if self.simulation_s_parameters.get("zmax") == None:
            self.simulation_s_parameters["zmax"] = max(
                inputsimdata_maxS["s_residual"].max(),
                inputsimdata_minS["s_residual"].max(),
            )
        ...
        tmin = self.simulation_s_parameters.get("tmin")
        tmax = self.simulation_s_parameters.get("tmax")
        itrans = self.simulation_s_parameters.get("itrans")
        ktype = self.simulation_s_parameters.get("ktype")
        nx = self.simulation_s_parameters.get("nx")
        ny = self.simulation_s_parameters.get("ny")
        xmn = self.simulation_s_parameters.get("xmn")
        ymn = self.simulation_s_parameters.get("ymn")
        xsiz = self.simulation_s_parameters.get("xsiz")
        ysiz = self.simulation_s_parameters.get("ysiz")
        zmin = self.simulation_s_parameters.get("zmin")
        zmax = self.simulation_s_parameters.get("zmax")
        nxdis = self.simulation_s_parameters.get("nxdis")
        nydis = self.simulation_s_parameters.get("nydis")
        ndmin = self.simulation_s_parameters.get("ndmin")
        ndmax = self.simulation_s_parameters.get("ndmax")
        radius = self.simulation_s_parameters.get("radius")

        if geostats is None:
            raise Exception("geostats is not installed")
        s_min_simulation = geostats.sgsim(
            inputsimdata_minS,
            "coord1",
            "ref_coord",
            "s_residual",
            ...
            colocorr=0.0,
            sec_map=0,
            vario=self.simulation_s_variogram,
        )

        s_max_simulation = geostats.sgsim(
            inputsimdata_maxS,
            "coord1",
            "ref_coord",
            "s_residual",
        ...
        simulation_s_thresholds = pd.DataFrame(
            columns=[
                "coord1",
                "min_s_residual",
                "min_s_threshold",
        ...
        model_conceptual_s = self.lateral_extent_model(
            simulation_s_thresholds, minP=minP, maxP=maxP, minS=minS, maxS=maxS
        )
        simulation_s_thresholds["conceptual_mins"] = model_conceptual_s[:, 1]
        simulation_s_thresholds["min_s_residual"] = s_min_simulation[1]
        simulation_s_thresholds["min_s_threshold"] = (
            model_conceptual_s[:, 1] - s_min_simulation[1]
        )

        simulation_s_thresholds["conceptual_maxs"] = model_conceptual_s[:, 0]
        simulation_s_thresholds["max_s_residual"] = s_max_simulation[1]
        simulation_s_thresholds["max_s_threshold"] = (
            model_conceptual_s[:, 0] - s_max_simulation[1]
        )
