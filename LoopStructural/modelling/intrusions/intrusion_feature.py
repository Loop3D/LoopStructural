...
class IntrusionFeature(GeologicalFeature):
    ...
    def add_marginal_fault(self, fault):
        """
        This function adds a marginal fault to the intrusion. 
        It is for plutons whose emplacement is controlled by marginal faults
        that generates roof uplift or floor depression.
    ...
    def evaluate_value_with_SGS(self, points):
        ...
        """
        CURRENTLY NOT BEING USED / SEPT 2022
    ...
    def interpolated_lateral_thresholds(self, points_coord1):
        ...
        # min side
        minL_residual_interpolator = Rbf(minL_inputdata_coord1, minL_inputdata_residual, function = 'linear')
        minL_conceptual_model = conceptual_model(points_coord_df, minP=minP, maxP=maxP, minS=minL, maxS=maxL)[:,1]
        ...
        # max side
        maxL_residual_interpolator = Rbf(maxL_inputdata_coord1, maxL_inputdata_residual, function = 'linear')
        maxL_conceptual_model = conceptual_model(points_coord_df, minP=minP, maxP=maxP, minS=minL, maxS=maxL)[:,0]
    ...
    def interpolated_vertical_thresholds(self, points_coord1, points_coord2):
        ...
        # max growth
        maxG_residual_interpolator = Rbf(maxG_inputdata_coord1, maxG_inputdata_coord2, maxG_inputdata_residual, function = function_rbf)
        maxG_conceptual_model = conceptual_model(points_df, minP=minP, maxP=maxP, minS=minL, maxS=maxL, mean_growth = mean_G, vertex = vertex)[:,1]

        maxG_minP = np.min(maxG_inputdata_coord1)
        maxG_minP_value = maxG_residual_interpolator(
            maxG_minP, maxG_inputdata_coord2[np.where(maxG_inputdata_coord1==maxG_minP)][0]
            )
        maxG_maxP = np.max(maxG_inputdata_coord1)
        maxG_maxP_value = maxG_residual_interpolator(
            maxG_maxP, maxG_inputdata_coord2[np.where(maxG_inputdata_coord1==maxG_maxP)][0]
            )
        maxG_minL = np.min(maxG_inputdata_coord2)
        maxG_minL_value = maxG_residual_interpolator(
            maxG_inputdata_coord1[np.where(maxG_inputdata_coord2==maxG_minL)][0], maxG_minL
            )
        maxG_maxL = np.max(maxG_inputdata_coord2)
        maxG_maxL_value = maxG_residual_interpolator(
            maxG_inputdata_coord1[np.where(maxG_inputdata_coord2==maxG_maxL)][0], maxG_maxL
            )
        ...
        minG_interpolator = Rbf(minG_inputdata_coord1, minG_inputdata_coord2, minG_inputdata_coord0, function = function_rbf)
    ...
    def evaluate_value(self, points):
        ...
        if simulated_s_data is None:
            ...
            thresholds, residuals, conceptual = self.interpolated_lateral_thresholds(intrusion_coord1_pts)
        ...
        if simulation_g_data is None:
            ...
            thresholds, residuals, conceptual = self.interpolated_vertical_thresholds(intrusion_coord1_pts, intrusion_coord2_pts)
