...
class IntrusionFeature():
    ...
    def evaluate_value(self, points):
        ...
        if simulation_s_data is None:
            ...
            pi_p = abs(intrusion_coord1_pts[:,None] - simulation_s_data_coord1[None,:])
            ...
            indicator_fxS_boolean = (s_minside_threshold <= intrusion_coord2_pts) * (intrusion_coord2_pts <= s_maxside_threshold)
        ...
        if simulation_g_data is None:
            ...
            # find index of closest value to each point being evaluated, and assign simulated s thresholds
            pi_p = abs(intrusion_coord1_pts[:,None] - simulation_g_data_coord1[None,:]) # p_points - p_simulated_data
            si_s = abs(intrusion_coord2_pts[:,None] - simulation_g_data_coord2[None,:])

            s_min = np.around(si_s/si_s.min(axis = 1)[:,None],2) # flag with 1 the minimum value
            p_min = np.around(pi_p/pi_p.min(axis = 1)[:,None],2) # flag with 1 the minimum value
            ...
            indicator_fxG_boolean = (g_minside_threshold <= intrusion_coord0_pts) * (intrusion_coord0_pts <= g_maxside_threshold)
