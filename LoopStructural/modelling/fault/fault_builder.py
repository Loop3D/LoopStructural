        ...
        if fault_center is not None:
            if influence_distance is not None:
                ...
                data = data.append(pd.DataFrame(
                    ...
                    columns=['X','Y','Z','feature_name','val','coord']),
                    ignore_index=True)
                data = data.append(pd.DataFrame(
            ...
            data = data.append(pd.DataFrame(
                ...
                columns=['X','Y','Z','feature_name','nx','ny','nz','val','coord']),
                ignore_index=True)
        # add strike vector to constraint fault extent
            data = data.append(pd.DataFrame(
                [[fault_center[0],fault_center[1],fault_center[2],self.name,strike_vector[0],strike_vector[1],strike_vector[2],2]],
                columns=['X','Y','Z','feature_name','nx','ny','nz','coord']),
                ignore_index=True)
