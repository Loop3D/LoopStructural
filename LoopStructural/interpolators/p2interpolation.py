...
class P2Interpolator(DiscreteInterpolator):
    ...
    def minimise_grad_steepness(self, sten =0.0, w=0.1):
    ...
    def minimize_edge_jumps(self,stren,w=0.1,maxmdDist=None): #NOTE: imposes \phi_T1(xi)-\phi_T2(xi) dot n =0
        ...
        v1 = tmesh.nodes[tmesh.edges][:,0,:]
        v2 = tmesh.nodes[tmesh.edges][:,1,:]
        ncp = 2
        ...
        cp1_Dt, cp1_tri1 = tmesh.evaluate_shape_derivatives(cp[:,0],elements=tmesh.edge_relationships[:,0])
        cp1_Dn, cp1_tri2 = tmesh.evaluate_shape_derivatives(cp[:,0],elements=tmesh.edge_relationships[:,1])
        cp2_Dt, cp2_tri1 = tmesh.evaluate_shape_derivatives(cp[:,1],elements=tmesh.edge_relationships[:,0])
        cp2_Dn, cp2_tri2 = tmesh.evaluate_shape_derivatives(cp[:,1],elements=tmesh.edge_relationships[:,1])
        ...
        # get vertex indexes
        tri_cp1 = np.hstack([tmesh.elements[cp1_tri1],tmesh.elements[cp1_tri2]])
        tri_cp2 = np.hstack([tmesh.elements[cp2_tri1],tmesh.elements[cp2_tri2]])
        # add cp1 and cp2 to the least squares system
        p2.add_constraints_to_least_squares(const_cp1*e_len[:,None]*w,np.zeros(const_cp1.shape[0]),tri_cp1, name='edge jump cp1')
