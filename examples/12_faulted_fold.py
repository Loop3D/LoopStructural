...
from LoopStructural.visualisation.rotation_angle_plotter import \
    RotationAnglePlotter

# ### Load data from shapefile
# We use geopandas to load the objects from a shapefile. In this case we use
# a geopackage with three
# shapefile layers:
# * points - value data
...
points = geopandas.read_file(
    '/home/lgrose/lachlan.grose@monash.edu/Loop/case_study_data/faulted_fold'
    '/data2.gpkg',
    layer='points')
...
fault_frame_builder = StructuralFrameBuilder(
    ...
    mesh=mesh,
    name='fault_frame')
# Interfacing with dataframes should be done using a convenience wrapper
# function
...
for i, r in points.iterrows():
    if r['label'] == 'fault':
        ...
        if 'z' in r:
            z = r['z']
        print(r)
        fault_frame_builder.add_point([xy[0][0], xy[1][0], z], r['value'],
                                      itype='gx')
# #         fault_frame_builder.add_point([xy[0][0],xy[1][0],z],r['value'],
# itype='gy')
#
# # # We define weights for the orthogonal constraint and the regularisation
# constraints. The solver to
...
for i, r in orientations.iterrows():
    if r['label'] == 'fold':
        ...
        if 'z' in r:
            z = r['z']
        fold_frame_builder.add_strike_and_dip([xy[0][0], xy[1][0], z],
                                              r['strike'], r['dip'],
                                              itype='gx')
for i, r in orientations.iterrows():
...
# # We need to create an empty fold event that links our fold frame to the
# fold event so that it can
# #  given to the fold interpolator.
fold = FoldEvent(faulted_fold_frame, None, None)
# #
# # # ### Create a DiscreteFoldInterpolator object
# # # The DiscreteFoldInterpolator is a daughter class of the
# PiecewiseLinearInterpolator that
# # # uses a fold event and a mesh to define additional constraints in the
# least squares system.
stratigraphy_interpolator = DFI(mesh, fold)
# #
# # # ### Build the stratigraphy geological feature
# # # We can build the stratigraphy geological feature using the fold
# interpolator object and
...
for i, r in orientations.iterrows():
    if r['label'] == 's0':
        ...
        if 'z' in r:
            z = r['z']
        stratigraphy_builder.add_strike_and_dip([xy[0][0], xy[1][0], z],
                                                r['strike'], r['dip'])
strati = stratigraphy_builder.build()
# viewer.plot_vector_data(strati.support.interpolator.get_gradient_control(
# )[:,:3],
...
# # The fold axis can be queried for any location in the model. The axis is
# a unit vector and
# # does not need to be normalised here. THe fold limb rotation angle is
# calculated by finding
# # the angle between the folded foliation and the axial foliation in the
# fold frame. The calculated
# # axis can be used by passing an N,3 array of the fold axis for every N
# locations. Or if no
# # axis is specified the local intersection lineation is used.
axis = fold.get_fold_axis_orientation(xyz)
flr = faulted_fold_frame.calculate_fold_limb_rotation(np.hstack([xyz, s0g]),
                                                      axis=axis)
...
rbf_fold_limb = Rbf(s1, np.zeros(s1.shape), np.zeros(s1.shape), flr_tan,
                    ...
                    epsilon=guess[1],
                    smooth=0.05)
xi = np.linspace(faulted_fold_frame.features[0].min(),
                 faulted_fold_frame.features[0].max(), 1000)


def fold_limb_rotation(x):
    return np.rad2deg(
        np.arctan(rbf_fold_limb(x, np.zeros(x.shape), np.zeros(x.shape))))
...
# # rotation_plots.add_fold_limb_curve(np.rad2deg(np.arctan(rbf_fold_limb(
# xi,np.zeros(1000),np.zeros(1000)))), xi)
# #
# # plt.show()
# # rotation_plots.add_fold_axis_curve(np.rad2deg(np.arctan(rbf_fold_axis(
# xi,np.zeros(1000),np.zeros(1000)))), xi)
rotation_plots.add_fold_limb_data(flr, s1)
rotation_plots.add_limb_svariogram(limb_svariogram)
# # rotation_plots.add_fold_limb_curve(np.rad2deg(np.arctan(rbf_fold_limb(
...
# # # **get_gradient_control()** returns an array N,6 array with the xyz and
# normal vector
# # # components of the gradient control points. We can use these arrays to
# calculate the
# # # fold axis using the intersection lineation between the axial foliation
# and the normal
# # # to the folded foliation. The fold axis rotation angle can then be
# calculated by finding
# # # the angle between the fold axis direction field and the intersection
# lineation. A
# # # SVariogram can then be used to automatically pick the wavelength of
# the fold
# #
...
# # # We use SciPy's RBF to interpolate the fold rotation angles. A
# smoothing parameter
...
# #
# # # We create a wrapper function for the fold axis rotation angle that can
# be given to the **FoldEvent**
# # # object so that it can calculate the fold rotation angle for any value
# of the fold frame.
# #
# # def fold_axis_rotation(x):
...
# #
# # # ### Evaluate the fold limb rotation angle
# # # The fold axis can be queried for any location in the model. The axis
...
# #                     epsilon=guess[0],
# #                     smooth=0.05)
# # xi = np.linspace(f1_frame.features[0].min(),f1_frame.features[0].max(),
# 1000)
# # def fold_limb_rotation(x):
# #     return np.rad2deg(np.arctan(rbf_fold_limb(x,np.zeros(x.shape),
# np.zeros(x.shape))))
# #
# # # The fold rotation angle function is added to the **FoldEvent**
# # fold.fold_limb_rotation = fold_limb_rotation
...
# # # The DiscreteFoldInterpolator has the ability to weigh the different
# geometrical properties of the
# # # fold differently in the least squares system. Much the same as data
# points and regularisation are
# # # handled differently for standard interpolation.
# # # * **fold_orientation** is the weight of the dot product between a
# # # * **fold_axis** is the weight of the dot product between the fold axis
# and the interpolated foliation
# # # * **fold_normalisation**
# # # * **fold_regularisation** is the fold regularisation term that ensures
# the fold is smooth along strike
# # # These weights need to be added to a dictionary and then passed to the
# feature builder along with the fold
# # fold_weights = {}
...
# #
# # # ### Visualising Results
# # # We can visualise the surfaces for isovalues of the fold frame or
# interpolated surface.
# # # Normal vectors to the scalar fields can be visualised for evaluation
# points (if too many
# # # points are chosen it can be difficult to see the locations. We suggest
# slicing numpy
...
viewer.add_vector_data(
    fault_frame.features[0].support.interpolator.get_gradient_constraints()[:,
    :3],
    fault_frame.features[0].support.interpolator.get_gradient_constraints()[:,
    3:],
...
viewer.add_vector_data(
    fault_frame.features[1].support.interpolator.get_gradient_constraints()[:,
    :3],
    fault_frame.features[1].support.interpolator.get_gradient_constraints()[:,
    3:],
    "gy_grad",
...
viewer.add_isosurface(faulted_fold_frame.features[0], colour='purple',
                      isovalue=0)
viewer.add_value_data(
    fault_frame.features[0].support.interpolator.get_value_constraints()[:,
    :3],
    fault_frame.features[0].support.interpolator.get_value_constraints()[:, 3],
    'fault_data')
# viewer.plot_value_data(strati.support.interpolator.get_control_points()[:,
# :3],
#                        strati.support.interpolator.get_control_points()[:,3],
#                        'strati_data')
# # viewer.plot_isosurface(fold_frame.features[1],  colour='red')
# # viewer.plot_vector_data(fold_frame.features[
# 0].support.interpolator.get_gradient_control()[:,:3],
# #                         fold_frame.features[
# 0].support.interpolator.get_gradient_control()[:,3:],
# #                         "gx_grad")
# # viewer.plot_vector_data(fold_frame.features[
# 1].support.interpolator.get_gradient_control()[:,:3],
# #                         fold_frame.features[
# 1].support.interpolator.get_gradient_control()[:,3:],
# #                         "gy_grad",
# #                         colour='red')
