"""
============================================================
4c. Comparing scipy's RBF interpolator to LoopStructural 2D
============================================================
...
p2_interpolator.add_value_constraints(w=1.0)
# setup_interpolator()'s default regularisation for P2 also calls
# minimise_grad_steepness(), which relies on second-derivative shape
# function code that was never finished for the 2D quadratic element
# (P2Unstructured2d.evaluate_shape_d2 references an attribute, self.hN,
# that's never set anywhere) - so here we add the same edge-jump
# smoothing P1 uses directly instead of going through
# setup_interpolator(). Without the missing curvature term this is
# noticeably weaker regularisation than P1 gets, which shows up below -
# see the discussion at the end of this example.
p2_interpolator.minimise_edge_jumps(w=1.0)
...
# * P2's quadratic shape functions can in principle follow curved
#   structure with a coarser mesh than P1 needs (this is exactly what
#   was verified when P2's 2D bounding-box construction was added), but
#   its RMSE above is noticeably worse than P1's. That's a real, current
#   limitation rather than a modelling choice: P1's regularisation and
#   P2's both minimise jumps in the gradient across element edges, but
#   P2 is also meant to add a curvature-minimising term
#   (``minimise_grad_steepness``) that P1 doesn't need. That term's 2D
#   implementation was never finished, so P2 here is running with
#   weaker regularisation than it's designed for - worth knowing before
#   reaching for P2 on sparse 2D data.
#
