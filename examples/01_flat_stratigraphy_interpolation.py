...
# ### GeologicalFeatureInterpolator
# LoopStructural uses an abstract class representation of geological
# objects. A **GeologicalFeature** is
# an abstract class that represents a geological object within the model. A
# **GeologicalFeature**
# can be evaluated for the gradient or scalar value at any location within
# the model.
# It can be simply a wrapper for a scalar field, or could be a piecewise
# combination of scalar
# fields.
# The **GeologicalFeature** needs to be built using a builder class,
# this class manages the data,
# interpolator and any geological structures. The
# **GeologicalFeatureInterpolator** provides a way
# of linking any LoopStructural interpolator to a **GeologicalFeature**. The
# GeologicalFeatureInterpolator
# needs to be given an interpolator object which should be a daughter class
# of a
# GeologicalInterpolator and a name to give the feature that is being built.
# The name is simply used
# as an identifier for the interpolated properties when exported into file
# formats or for visualisation.
# There are currently has two interpolator options available , a piecewise
# linear interpolator using
# a tetrahedral mesh where the roughness of the interpolated surfaces is
# minimised.
# A finite difference interpolator where the gaussian curvature is minimised.
# The GeologicalFeatureInterpolator can be given value constraints
# **add_point(position,value)** or
# gradient constraints **add_strike_and_dip**(pos,strike,dip)
# The resulting feature can be interpolated by calling **build**(kwargs)
# where the kwargs are
# passed to the interpolator you have chosen e.g. which solver to use,
# what weights etc..
...
feature = feature_builder.build(
    solver='cg',
    tol=1e-10,
    # need to add tolerance constraint to cg if the scalar values are low
    cgw=0.1)

# Export the input data and the model domain to a text file so it can be
# imported into gocad
# np.savetxt("01_gradient.txt",
# feature_builder.interpolator.get_gradient_control())
...
# LoopStructural uses Lavavu for visualising objects. The LavaVuModelViewer
# class interfaces between lavavu and LoopStructural
...
# print(feature.support.get_node_values()[np.isnan(
# feature.support.get_node_values())==True])
