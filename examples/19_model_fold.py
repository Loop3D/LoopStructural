...
# determine the number of unique surfaces in the model from the input data
# and then calculate isosurfaces for this
# unique = np.unique(stratigraphy_builder.interpolator.get_control_points()[
# :,3])
# viewer.add_scalar_field(folded_model.bounding_box,(38,55,30),
#                       'box',
#                      paint_with=folded_foliation,
#                      cmap='prism')
#     viewer.add_vector_data(stratigraphy.get_interpolator(
#     ).get_norm_constraints()[:,:3],
#     stratigraphy.interpolator.get_norm_constraints()[:,:3],'data')
