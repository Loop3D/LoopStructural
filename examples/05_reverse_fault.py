...
fault.add_strike_dip_and_value([10, 0, -2], strike=90., dip=50., val=0,
                               itype='gx')
fault.add_strike_dip_and_value([5, 15, 0], strike=90., dip=5., val=0,
                               itype='gx')
fault.add_strike_dip_and_value([15, 15, 0], strike=90., dip=5., val=0,
                               itype='gx')
fault.add_strike_dip_and_value([10, 15, 0], strike=90., dip=5., val=0,
                               itype='gx')
fault.add_strike_dip_and_value([5, -10, -10], strike=45, dip=10, val=0,
                               itype='gx')
fault.add_strike_dip_and_value([15, -10, -10], strike=45, dip=10, val=0,
                               itype='gx')
fault.add_strike_dip_and_value([10, -10, -10], strike=45, dip=10, val=0,
                               itype='gx')
...
slices = [-4, -2, 0]
viewer.add_vector_data(fault_interpolator.get_gradient_constraints()[:, :3],
                       fault_interpolator.get_gradient_constraints()[:, 3:],
                       "grd")
viewer.add_isosurface(fault_frame.features[0], isovalue=0, colour='green')
locations = mesh.barycentre[::20, :]
viewer.add_vector_field(fault_frame.features[1], locations=locations,
                        colour='red')
