...
def test_outside_box_normal():
    for interpolator in ['PLI','FDI']:
        ...
        data = pd.DataFrame([[0.5,0.5,0.5,0,1.,0.,'strati'],
                             [1.5,0.5,0.5,0,1.,0.,'strati'],
                             [0.5,1.5,1.5,0,1.,0.,'strati']],columns=['X','Y','Z','nx','ny','nz','feature_name'])
        model.data = data
...
def test_outside_box_value():
    for interpolator in ['PLI','FDI']:
        ...
        model = GeologicalModel(np.zeros(3),np.ones(3))
        data = pd.DataFrame([[0.5,0.5,0.5,0,'strati'],
                             [1.5,0.5,0.5,0,'strati'],
                             [0.5,1.5,1.5,0,'strati']],columns=['X','Y','Z','val','feature_name'])
        model.data = data
        model.create_and_add_foliation('strati',interpolatortype=interpolator)
        model.update()
