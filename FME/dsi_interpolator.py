...
from .dsi_helper import compute_gradient_ortho_constraint,compute_cg_regularisation_constraint
class DSI(GeologicalInterpolator):
    ...
    def calculate_constant_gradient_with_element_weighting(self,weighting):
        ...
        for i, t in enumerate(self.mesh.elements):
            ...
            for n in self.mesh.get_neighbors(i):
                ...
                idc, cstr = compute_cg_regularisation_constraint(d_t,d_n,self.mesh.elements[i],self.mesh.elements[n],self.mesh.nodes)
                idc = np.array(idc)
                cstr = np.array(cstr)
