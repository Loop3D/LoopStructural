...
def solver_diagnostics(
    ...
    ##
    # Preprocess A
    if not (sparse.isspmatrix_csr(A) or sparse.isspmatrix_bsr(A)):
        try:
    ...
    # Detect symmetry
    if symmetry is None:
        if pyamg.util.linalg.ishermitian(A, fast_check=True):
            symmetry = 'hermitian'
        else:
    ...
    if B_list is None:
        B_list = [(np.ones((A.shape[0], 1), dtype=A.dtype),
    ...
    if strength_list is None:
        strength_list = [
            ('symmetric', {
                'theta': 0.0}), ('evolution', {
                    'k': 2, 'proj_type': 'l2', 'epsilon': 2.0}), ('evolution', {
                        'k': 2, 'proj_type': 'l2', 'epsilon': 4.0})]
    ...
    if smooth_list is None:
        if definiteness == 'positive' and (
            ...
            if solver.__name__ == 'smoothed_aggregation_solver':
                smooth_list = [
                    'jacobi', ('jacobi', {
            ...
            smooth_list = [
                ('energy', {
                    'krylov': 'gmres', 'maxiter': 2, 'degree': 1, 'weighting': 'local'}), ('energy', {
                        'krylov': 'gmres', 'maxiter': 3, 'degree': 2, 'weighting': 'local'}), ('energy', {
                            'krylov': 'gmres', 'maxiter': 4, 'degree': 3, 'weighting': 'local'})]
        else:
    ...
    if prepostsmoother_list is None:
        if symmetry == 'nonsymmetric' or definiteness == 'indefinite':
            prepostsmoother_list = [
                (('gauss_seidel_nr', {
                    'sweep': 'symmetric', 'iterations': 2}), ('gauss_seidel_nr', {
                        'sweep': 'symmetric', 'iterations': 2}))]
        else:
            prepostsmoother_list = [
                (('block_gauss_seidel', {
                    'sweep': 'symmetric', 'iterations': 1}), ('block_gauss_seidel', {
    ...
    if improve_candidates_list is None:
        if symmetry == 'nonsymmetric' or definiteness == 'indefinite':
            improve_candidates_list = [
                [('gauss_seidel_nr', {'sweep': 'symmetric', 'iterations': 4}), None], None]
        else:
            improve_candidates_list = [
                [('block_gauss_seidel', {'sweep': 'symmetric', 'iterations': 4}), None], None]
    ...
    if A.dtype == complex:
        x0 += 1.0j * np.random.rand(A.shape[0], 1)
    ...
    solver_descriptors = list(np.array(solver_descriptors)[indys])
    solver_args = list(np.array(solver_args)[indys])
    ...
    fptr.write(
        ...
        '****************************************************************\n\n')
    fptr.write(pyamg.util.utils.print_table(table))
    ...
    def to_string(a):
        if isinstance(a, type((1,))):
            return(str(a))
        elif isinstance(a, type('s')):
            return("\"%s\"" % a)
        else:
            return str(a)
    #
    ...
    fptr.write(
        ...
        '        strength=%s,\n' %
        to_string(
            solver_args[0]['strength']) +
        '        smooth=%s,\n' %
        to_string(
            solver_args[0]['smooth']) +
        '        improve_candidates=%s,\n' %
        to_string(
            solver_args[0]['improve_candidates']) +
        '        aggregate=%s,\n' %
        to_string(
            solver_args[0]['aggregate']) +
        '        presmoother=%s,\n' %
        to_string(
            solver_args[0]['presmoother']) +
        '        postsmoother=%s,\n' %
        to_string(
            solver_args[0]['postsmoother']) +
        '        max_levels=%s,\n' %
        to_string(
            solver_args[0]['max_levels']) +
        '        max_coarse=%s,\n' %
        to_string(
            solver_args[0]['max_coarse']) +
        '        coarse_solver=%s)\n\n' %
        to_string(
            solver_args[0]['coarse_solver']))
    fptr.write('    ##\n    # Solve system\n')
    fptr.write('    res = []\n')
    fptr.write(
        '    x = ml.solve(b, x0=x0, tol=%s, residuals=res, accel=%s, maxiter=%s, cycle=%s)\n' %
        (to_string(
            solver_args[0]['tol']), to_string(
            solver_args[0]['accel']), to_string(
                solver_args[0]['maxiter']), to_string(
                    solver_args[0]['cycle'])))
